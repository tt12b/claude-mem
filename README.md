...
대시보드 API(`/api/*`)에는 **인증이 없습니다.** 열려 있는 곳에서는 누구나

- 저장된 작업 기록을 전부 읽고,
- 질문을 던져 Gemini 무료 한도를 소모시키고,
- 기억한 메모를 지우고, 사용 모델을 바꿀 수 있습니다.

`/v1/*`(훅이 쓰는 경로)는 API 키로 보호되지만 대시보드 경로는 아닙니다.
그러니 **신뢰할 수 있는 네트워크에만 노출하세요.** `.env` 에서 바인딩할
인터페이스를 하나로 고정하는 것이 가장 확실합니다.

```dotenv
# Tailscale 로만 열기
CLAUDE_MEM_BIND_ADDRESS=100.x.y.z
# 또는 같은 PC 에서만
CLAUDE_MEM_BIND_ADDRESS=127.0.0.1
```

바꾼 뒤 `docker compose up -d` 로 다시 올리면 적용됩니다. 지정한 주소가
기동 시점에 없으면 컨테이너가 뜨지 않으므로, Tailscale 주소를 쓸 때는
Tailscale 이 먼저 올라오는지 확인하세요.
